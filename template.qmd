...
```{julia}
temp_lat = temp10_lat[:14] #All of the lat data from the different temp datasets are the same, so I represent the lat for the whole temp data just as one of lat variables I made earlier
...
temp = combined_aggregated_temps[30:41,:14,:]
```
:::{.callout-note}
...
## 2.5 Filtering Data for Desired Latitude & Longitude Range
```{julia}
function subset_temperature_data(temp_lat, temp_lon, temp_data, target_lat_range, target_lon_range)
    lat_indices = findall(x -> x >= target_lat_range[1] && x <= target_lat_range[2], temp_lat)
    lon_indices = findall(x -> x >= target_lon_range[1] && x <= target_lon_range[2], temp_lon)

    subset_temp_lat = temp_lat[lat_indices]
    subset_temp_lon = temp_lon[lon_indices]

    subset_temp_data = temp_data[lat_indices, lon_indices, :]

    return subset_temp_lat, subset_temp_lon, subset_temp_data
end
```
```{julia}
# Specify the desired latitude and longitude ranges for temperature data
target_lat_range = (25.0f0, 37.0f0)
target_lon_range = (-102.0f0, -90.0f0)

# Call the function to get the subset of temperature data
subset_temp_lat, subset_temp_lon, subset_temp = subset_temperature_data(
    temp_lat, temp_lon, temp, target_lat_range, target_lon_range
)
```
```{julia}
# Update temperature variables with the subset
temp_lat = subset_temp_lat
temp_lon = subset_temp_lon
temp = subset_temp
```
...
```{julia}
avg_precip =ustrip.(u"mm", [mean(skipmissing(precip_train[:, :, t])) for t in 1:size(precip_train, 4)])
...
plot(p1, p2, p3; size=(1800, 600), link=:both)
```
