...
# 7. Model #2: Random Forests Model

```{julia}
# Making temp_mat_train and temp_mat_test are preprocessed temperature data
X_train1 = temp_mat_train'
X_test1 = temp_mat_test'

# Making avg_precip my target variable (response variable)
y_train1 = avg_precip
```
```{julia}
...
model = build_forest(y_train1, X_train1, n_trees)
```
```{julia}
# Make predictions on the testing data
y_pred = apply_forest(model, X_test1)
```

# 8. Quantile-Quantile Mapping: Bias Correction

```{julia}
function qq_mapping(predicted_data, observed_data)
    # Flatten the arrays
    predicted_flat = vec(skipmissing(predicted_data))
    observed_flat = vec(skipmissing(observed_data))

    # Sort the data
    sort_predicted = sort(predicted_flat)
    sort_observed = sort(observed_flat)

    # Calculate quantiles
    quantiles_predicted = collect(1:length(sort_predicted)) / length(sort_predicted)
    quantiles_observed = collect(1:length(sort_observed)) / length(sort_observed)

    # Map quantiles using interpolation
    mapped_values = interp1(quantiles_predicted, sort_predicted, quantiles_observed)

    # Reshape the mapped values to the original shape
    mapped_precipitation = reshape(mapped_values, size(observed_data))

    return mapped_precipitation
end

# Apply Q-Q mapping
mapped_precipitation = qq_mapping(precip_pred, precip_test)

# Visualization of Q-Q mapping results
heatmap(precip_lon, precip_lat, mapped_precipitation[:, :, 1]'; xlabel="Longitude", ylabel="Latitude", title="Mapped Precipitation")
```
```{julia}
#Generating Quantiles
quantiles_obs = quantile(precip_test, [0.25, 0.5, 0.75])
quantiles_pred = quantile(y_pred, [0.25, 0.5, 0.75])
```

```{julia}
#Quantile Mapping
function quantile_mapping(y_pred, quantiles_pred, quantiles_obs)
    mapped_values = similar(y_pred)
    for i in 1:length(y_pred)
        quantile_index = searchsortedfirst(quantiles_pred, y_pred[i])
        mapped_values[i] = quantiles_obs[quantile_index]
    end
    return mapped_values
end

y_pred_mapped = quantile_mapping(y_pred, quantiles_pred, quantiles_obs)
```
